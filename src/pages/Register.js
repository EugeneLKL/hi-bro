...
const Register = () => {
    ...
    const genderValidator = (_, value, callback) => {
        if (!value) {
            return callback(
                new Error(`Please select one option from 'Gender'!`)
            );
        }
        else {
            return callback();
    ...
    const onFinish = async (values) => {
        ...
        if (checkPhone.data === false && checkEmail.data === false) {
            message.warning(" It looks like you already have an account. Please try log in instead.");
        }
    ...
    }, [phoneRegistered], [emailRegistered]);
    ...
    return (
        <div className='register-form'>
            ...
            >
                ...
                // onChange for combining prefix number and phone number entered
                >
                    <Input addonBefore={prefixSelector} style={{ width: '100%' }} ref={phoneInputRef} />
                </Form.Item>
